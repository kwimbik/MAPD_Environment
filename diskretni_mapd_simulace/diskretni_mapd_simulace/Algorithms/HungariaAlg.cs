...
{
    public sealed class HungariaAlg
    {
        ...
        public HungariaAlg(int[,] costMatrix)
        {
            _costMatrix = StabilizeMAtrix(costMatrix);
        }
        ...
        private int[,] StabilizeMAtrix(int[,] costMatrix)
        {
            int _rows= costMatrix.GetLength(0);
            int _columns = costMatrix.GetLength(1);
            int[,] stabilizedMatrix = new int[Math.Max(_rows, _columns), Math.Max(_rows, _columns)];

            for (int i = 0; i < _rows; i++)
            {
                for (int j = 0; j < _columns; j++)
                {
                    stabilizedMatrix[i, j] = costMatrix[i, j];
                }
            }
            ...
            if (_rows > _columns)
            {
                for (int i = 0 ; i < _rows; i++)
                {
                    for (int j = _columns; j < _rows; j++)
                    {
                        stabilizedMatrix[i, j] = int.MaxValue;
                    }
                }
            }
